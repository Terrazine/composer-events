...
//        });
    ...
    public function makeSureEventsAreRegistered() {
        ...
        if (!in_array('@php artisan terrazine:composer-events:pre-autoload-dump --ansi || true', $composer->scripts->{'pre-autoload-dump'})) {
            array_push($composer->scripts->{'pre-autoload-dump'}, '@php artisan terrazine:composer-events:pre-autoload-dump --ansi || true');
            $changed = true;
        }

        if (!in_array('@php artisan terrazine:composer-events:post-autoload-dump --ansi || true', $composer->scripts->{'post-autoload-dump'})) {
            array_push($composer->scripts->{'post-autoload-dump'}, '@php artisan terrazine:composer-events:post-autoload-dump --ansi || true');
            $changed = true;
        }
